...
async function apiGet(pathname) {
  ...
  if (!res.ok)
    throw new Error(
      `Confluence API ${res.status} ${res.statusText} for ${pathname}`,
    );
  return res.json();
}
...
async function fetchComments(pageId) {
  const [inline, footer] = await Promise.all([
    apiGet(
      `/api/v2/pages/${pageId}/inline-comments?limit=100&body-format=atlas_doc_format`,
    ).catch(() => ({ results: [] })),
    apiGet(
      `/api/v2/pages/${pageId}/footer-comments?limit=100&body-format=atlas_doc_format`,
    ).catch(() => ({ results: [] })),
  ]);
...
async function cmdSync({ dryRun }) {
  ...
  for (const file of files) {
    ...
    if (nextStatus) {
      statusResult = applyStatusChange(
        { ...file, raw: nextContent },
        nextStatus,
      );
      if (statusResult.changed) {
        nextContent = statusResult.content;
        pageReport.statusChange = {
          from: statusResult.previous,
          to: statusResult.next,
        };
      }
    }
  ...
  for (const p of summary.pages) {
    ...
    if (p.statusChange)
      bits.push(`status ${p.statusChange.from || "—"} -> ${p.statusChange.to}`);
...
function cmdList() {
  ...
  for (const f of files)
    console.log(`${f.pageId}\t${f.data.status || "-"}\t${f.path}`);
}
...
async function main() {
  ...
  console.error("Usage:");
  console.error(
    "  node scripts/confluence-sync.js sync [--cwd=<path>] [--dry-run]",
  );
  console.error("  node scripts/confluence-sync.js list [--cwd=<path>]");
  console.error("");
  console.error(
    "  --cwd=<path>  Override the repo root. Defaults to script's parent.",
  );
  process.exit(2);
}
